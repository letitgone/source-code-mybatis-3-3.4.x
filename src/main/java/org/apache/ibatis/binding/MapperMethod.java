...
public class MapperMethod {
    ...
    private <K, V> Map<K, V> executeForMap(SqlSession sqlSession, Object[] args) {
        ...
        if (method.hasRowBounds()) {
            RowBounds rowBounds = method.extractRowBounds(args);
            result = sqlSession
                    .<K, V>selectMap(command.getName(), param, method.getMapKey(), rowBounds);
        } else {
        ...
        public SqlCommand(Configuration configuration, Class<?> mapperInterface, Method method) {
            ...
            if (ms == null) {
                if (method.getAnnotation(Flush.class) != null) {
                    ...
                    throw new BindingException(
                            "Invalid bound statement (not found): " + mapperInterface.getName()
                                    + "." + methodName);
                }
            } else {
                name = ms.getId();
        ...
        private MappedStatement resolveMappedStatement(Class<?> mapperInterface, String methodName,
            ...
            for (Class<?> superInterface : mapperInterface.getInterfaces()) {
                if (declaringClass.isAssignableFrom(superInterface)) {
                    MappedStatement ms =
                            resolveMappedStatement(superInterface, methodName, declaringClass,
                                    configuration);
                    if (ms != null) {
                        return ms;
                    }
            ...
            for (int i = 0; i < argTypes.length; i++) {
                if (paramType.isAssignableFrom(argTypes[i])) {
                    if (index == null) {
                        ...
                        throw new BindingException(
                                method.getName() + " cannot have multiple " + paramType
