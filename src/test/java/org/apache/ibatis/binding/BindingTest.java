...
public class BindingTest {
    ...
    public void shouldInsertAuthorWithSelectKey() {
        ...
        try {
            BoundAuthorMapper mapper = session.getMapper(BoundAuthorMapper.class);
            Author author =
                    new Author(-1, "cbegin", "******", "cbegin@nowhere.com", "N/A", Section.NEWS);
    ...
    public void shouldSelectListOfPostsLikeTwoParameters() {
        ...
        try {
            BoundBlogMapper mapper = session.getMapper(BoundBlogMapper.class);
            List<Post> posts =
                    mapper.selectPostsLikeSubjectAndBody(new RowBounds(1, 1), "%a%", "%a%");
            assertEquals(1, posts.size());
        } finally {
            session.close();
        }
    ...
    public void shouldSelectOneAuthorFromCache() {
        ...
        assertTrue("Same (cached) instance should be returned unless rollback is called.",
                author1 == author2);
    }
    ...
    public void shouldCacheMapperMethod() throws Exception {
        ...
        try {
            ...
            final MapperMethod cachedSelectBlog =
                    mapperProxyFactory.getMethodCache().get(selectBlog);
            ...
            assertSame(cachedSelectBlog, mapperProxyFactory.getMethodCache().get(selectBlog));
            assertTrue(mapperProxyFactory.getMethodCache()
                    .containsKey(selectBlogByIdUsingConstructor));
