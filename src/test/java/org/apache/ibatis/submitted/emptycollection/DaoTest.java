...
public class DaoTest {
    ...
    public void setUp() throws Exception {
        Reader reader = Resources.getResourceAsReader(
                "org/apache/ibatis/submitted/emptycollection/mybatis-config.xml");
    ...
    private void checkNonEmptyList(final List<TodoLists> actual) {
        ...
        Assert.assertEquals("a description",
                actual.get(0).getTodoItems().get(0).getDescription().trim());
        Assert.assertEquals(2, actual.get(0).getTodoItems().get(1).getOrder());
        Assert.assertEquals("a 2nd description",
        ...
        Assert.assertNotSame(actual.get(0).getTodoItems().get(0),
                actual.get(1).getTodoItems().get(0));
    }
}
